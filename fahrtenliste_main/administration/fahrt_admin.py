...
class FahrtAdminForm(forms.ModelForm):
    def clean_entfernung(self):
        ...
        #    if entfernung is not None and entfernung != kunde.adresse.entfernung:
        #        errors.append(
        #            ValidationError(
        #                "Die Entfernung weicht von der Entfernung der aktuellen Kunden Adresse ab. "
        #                f"Bitte auf {kunde.adresse.entfernung} km korrigieren"))
        if len(errors) > 0:
...
class FahrtAdmin(CompareVersionAdmin):
    ...
    def save_model(self, request, obj, form, change):
        if obj.adresse is None and obj.kunde is not None:
            # Bei Neuanlage wird die Adresse vorbelegt
            obj.adresse = obj.kunde.adresse

        if obj.entfernung is None and obj.kunde is not None:
